...
import { apiWithdrawList} from "../../../request/api";
...
const getWithdrawList = async () => {
  const res = await apiWithdrawList({...params})
  ...
  <div class="bg-[#F2F2F2] md:pt-0 pt-[46px] md:!h-[100vh] md:mb-0 md:bg-[#F5F9FF] h-[100vh] relative">
    ...
    <div class="md:w-[1200px] md:!mx-auto md:!bg-white md:mt-[20px] md:rounded-2xl md:px-[24px]">
      ...
      <div class="py-[20px] px-[10px] md:!pt-0 md:!px-0 mb:!pb-[20px] bg-white md:mx-auto md:rounded-2xl">
        ...
        <el-table
            :data="tableData"
            border style="width: 100%; table-layout: auto;" class="mt-[20px] !text-[12px]" :height="tableData.length ? '500' : 'auto'" empty-text="暂无提现记录" :header-cell-style=" { 'text-align': 'center' }">
          <el-table-column
              prop="create_time"
              label="时间"
              align="center"
          ...
          >
            <template #default="{row}">
              <span>{{ priceFormat(row.real_amount) }}</span>
            </template>
          </el-table-column>
          <el-table-column
              prop="status"
